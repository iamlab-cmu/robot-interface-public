...
where:
    ...
    -s Start iam-robolib on control PC (0 (default) / 1)
    
    ./start_control_pc.sh -i iam-space
    ./start_control_pc.sh -i iam-space -u iam-lab -p 12345678 -d ~/Documents/robot-interface -r 1 -s 1
    "
...
while getopts ':h:i:u:p:d:r:' option; do
  case "${option}" in
    h) echo "$usage"
       exit
       ;;
...
gnome-terminal -e "bash $start_iam_robolib_on_control_pc_path $control_pc_uname $control_pc_ip_address $workstation_ip_address $control_pc_use_password $control_pc_password $control_pc_robolib_path"
echo "Done"
sleep 3
else
...
gnome-terminal -e "bash $start_ros_action_lib_on_control_pc_path $control_pc_uname $control_pc_ip_address $workstation_ip_address $control_pc_use_password $control_pc_password $control_pc_robolib_path $robot_number"
sleep 3
echo "Done"
