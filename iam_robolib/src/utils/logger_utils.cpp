...
std::vector<std::string> LoggerUtils::all_logger_files() {
  std::string prefix = "robot_state_data_";
  std::vector<std::string> all_log_files;
  for (boost::filesystem::directory_entry& f : boost::filesystem::directory_iterator("./")) {
...
int LoggerUtils::integer_suffix_for_new_log_file()  {
  std::string prefix = "robot_state_data_";
  int curr_suffix = 0;
  for (boost::filesystem::directory_entry& f : boost::filesystem::directory_iterator("./")) {
    std::string filename = f.path().filename().string();
