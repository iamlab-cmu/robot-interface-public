...
void CartesianPoseSkill::execute_skill_on_franka(run_loop* run_loop,
  ...
  int log_counter = 0;
  std::array<double, 16> last_O_T_EE_c; 
  std::array<double, 16> final_O_T_EE;
  double D = 1.0;
  double tau = 0.0;
  ...
  std::function<franka::CartesianPose(const franka::RobotState&, franka::Duration)>
    ...
    if((time > 0.0 && done) || skill_termination_handler_end_time > 0.0) {
      ...
      if (skill_termination_handler_end_time == 0.0) {
        ...
        for(int i = 0; i < 3; i++) {
          ...
          initial_velocity[i] = robot_state.O_dP_EE_c[i];
      ...
      if (time - skill_termination_handler_end_time < D) {
        tau = std::min(std::max((time - skill_termination_handler_end_time) / D, 0.001), 1.0);
    ...
    last_O_T_EE_c = robot_state.O_T_EE_c;
    return desired_pose;
  };
