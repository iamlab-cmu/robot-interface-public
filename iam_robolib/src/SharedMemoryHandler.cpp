...
RunLoopProcessInfo* SharedMemoryHandler::getRunLoopProcessInfo() {
  return run_loop_info_;
}

boost::interprocess::interprocess_mutex* SharedMemoryHandler::getRunLoopProcessInfoMutex() {
  return run_loop_info_mutex_;
}

SharedBuffer SharedMemoryHandler::getTrajectoryGeneratorBuffer(int memory_region) {
  if (memory_region == 0) {
...
SharedBuffer SharedMemoryHandler::getFeedbackControllerBuffer(int memory_region) {
  if (memory_region == 0) {
...
SharedBuffer SharedMemoryHandler::getTerminationParametersBuffer(int memory_region) {
  if (memory_region == 0) {
...
SharedBuffer SharedMemoryHandler::getExecutionResultBuffer(int memory_region) {
  if (memory_region == 0) {
...
SharedBuffer SharedMemoryHandler::getFeedbackResultBuffer(int memory_region) {
  if (memory_region == 0) {
...
void SharedMemoryHandler::start() {
