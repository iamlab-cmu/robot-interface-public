...
void SinePoseTrajectoryGenerator::parse_parameters() {
...
void SinePoseTrajectoryGenerator::initialize_trajectory() {
  // assert(false);
}

void SinePoseTrajectoryGenerator::initialize_trajectory(const franka::RobotState &robot_state) {
...
void SinePoseTrajectoryGenerator::get_next_step() {
  ...
  if(!saved_full_trajectory_) {
    FILE * pFile = fopen ("bad_sine_trajectory.txt","w");
